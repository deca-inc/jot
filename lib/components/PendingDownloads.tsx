  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        >
          {pendingDownloads.length} paused download
          {pendingDownloads.length > 1 ? "s" : ""} • Tap to resume
        </Text>
      </View>
      ...
      {pendingDownloads.map((download) => {
        ...
        const ageInDays = Math.floor(
          (Date.now() - download.startedAt) / (1000 * 60 * 60 * 24),
        );
        ...
        const displayProgress =
          currentProgress !== undefined
            ...
            : download.bytesTotal > 0
              ? Math.round((download.bytesWritten / download.bytesTotal) * 100)
              : 0;
